...
float NoiseField::sqrt2 = sqrt(2.f);
...
void NoiseField::regenerateHash(int s)
{
    hash.resize(256);
    ...
    std::shuffle(hash.begin(), hash.end(), engine);
...
void NoiseField::generate(int s)
{
    ...
    for(int i = 0 ; i < nx ; i++)
    {
        for(int j = 0 ; j < ny ; j++)
        {
            //169 0
            //cout << "i= " << i << " j=" << j << endl;
            z[index(i,j)] = sumPerlinNoise(Vertex(i,j) + parameters.offset) * 0.5 + 0.5;
        }
    }
}

float NoiseField::perlinNoise(vec2 point)
{
...
float NoiseField::sumPerlinNoise(vec2 point)
{
    float sum = 0;
    float freq = parameters.frequence;
    float amp = parameters.amplitude;

    for (int i = 0 ; i < parameters.octaves ; i++) {
        sum += perlinNoise(point*freq)*amp;
